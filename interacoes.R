...
# Gráfico 2: Boxplots para ver distribuição
ggplot(df_interaction, aes(x = factor(smoke), y = wt, fill = race_group)) +
  geom_boxplot(alpha = 0.7) +
  facet_wrap(~race_group) +
  labs(x = "Fuma durante gravidez?", y = "Peso (onças)",
       title = "Distribuição do peso por fumo e raça")
...
ggplot(df_interaction, aes(x = factor(smoke), y = wt, fill = factor(smoke))) +
  geom_boxplot(alpha = 0.7) +
  labs(
    x = "Fuma durante gravidez?",
    y = "Peso (onças)",
    fill = "Fuma?",
...
# 1. Fuma ou não
ggplot(df, aes(x = factor(smoke), y = wt, fill = factor(smoke))) +
  geom_boxplot(alpha = 0.7) +
  labs(
...
# 2. Tempo de fumo
ggplot(df, aes(x = factor(time_group), y = wt, fill = factor(time_group))) +
  geom_boxplot(alpha = 0.7) +
  labs(
    x = "Tempo de fumo",
    y = "Peso ao nascer (onças)",
...
# 3. Número de cigarros
ggplot(df, aes(x = factor(number_group), y = wt, fill = factor(number_group))) +
  geom_boxplot(alpha = 0.7) +
  labs(
